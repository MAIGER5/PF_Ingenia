import React from 'react'
import { Box, Button, Grid, Typography,  } from '@mui/material'
import { Link } from 'react-router-dom'

export default function HomeCategorias() {
    let categorias=['Programacion', 'Seguridad Informatica', 'Data', 'Inteligencia Artificial', 'Java', 'Aplicaciones Moviles', 'Softwate', 'Aplicaicone Web','Videojuegos', 'Python', 'Redesd de Datos', 'Ofimatica']
  return (
   
    <Box sx={{marginTop:4, width:1600}}>
      <Typography variant="h5" color="primary"  sx={{marginBottom:4, marginTop:4,marginLeft:-169}}>Categorias</Typography>
      <Grid container spacing={3} sx={{marginLeft:3}} >
        {categorias.map(categori=>(
          <Grid item xs={3} sm={5} md={4} xl={2}>
            <Link to='/CategoryCourses'>
              <Button variant="contained" color="secondary" sx={{width:220 ,height:60}}>
                {categori}
              </Button>
            </Link>
          </Grid>
        ))}
     </Grid>
    </Box>
  )
}
